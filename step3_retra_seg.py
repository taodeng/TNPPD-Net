...
#------------------activatemodel----------------------
    ...
    model_precls = args.model_precls_path
    if model_precls != "":
        assert os.path.exists(model_precls), "weights file: '{}' not exist.".format(model_precls)
        weights_dict = torch.load(model_precls, map_location=device, weights_only=True)
        print(f"For pre_cls: {model.load_state_dict(weights_dict)}")

    frozen_layers = [model.all_level1, model.cls_level2, model.cls_level3, model.cls_blocks, model.cls_bn, model.cls_head,
                     model.fu_conv, model.fu_convfusion, model.fu_qkvfusion, 
                     model.seg_level2, model.seg_level3, model.seg_level4, model.seg_level5_1, model.seg_level5_2, 
                     model.seg_level2_0, model.seg_level3_0, model.seg_level4_0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_precls_path', type=str, default='')

    opt = parser.parse_args()

    save_path = f"./result/step3_retra_seg/{opt.exp_name}"
